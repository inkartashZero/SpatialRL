...
plt.rcParams.update({
    ...
    'ytick.labelsize': 12,
    'legend.fontsize': 12,
    'figure.dpi': 300         # Ensure high DPI for compression
})
...
def plot_continuous_track(
    ...
    and annotates the reward structure. Now optimized with a white theme.
    ...
    ax.set_xlabel("Position (cm)", color="black")

    # annotations (drawn over the colored track)
    for x, label, col in [
        (tw/2,     "LEFT\nLICK PORT\n(RED)",   "white"), # White text against red port
        (L/2,      "Track Body\n(BLACK)",      "white"), # White text against black track
    ...
    ax.axvline(tw,   color="black", lw=2.5, linestyle="--", alpha=0.8)
    ax.axvline(L-tw, color="black", lw=2.5, linestyle="--", alpha=0.8)
    ...
    # Darker reward fill for visibility on white
    ax2.fill_between(xs, rew, color="#00C49A", alpha=0.35) 
    ax2.axhline(0, color="black", lw=1.0)

    # step penalty line
    ax2.axhline(-0.005 * 100, color="#D00000", lw=2.0, linestyle=":",
                label=f"step_penalty×100 = {env.step_penalty*100:.2f}")
    ax2.axhline(env.lick_penalty, color="#E67E22", lw=2.0, linestyle="--",
                label=f"lick_penalty = {env.lick_penalty:.3f}")
    ax2.axhline(env.water_reward, color="#00C49A", lw=2.0, linestyle="-",
                label=f"water_reward = {env.water_reward:.2f}")

    ax2.set_xlim(0, L)
    ax2.set_ylim(-0.15, 1.2)
    ax2.set_yticks([]); ax2.tick_params(axis="x", colors="black")
    ax2.set_xlabel("Position (cm)", color="black")
    
    # Legend adapted for white background
    leg = ax2.legend(facecolor="white", edgecolor="black",
                     labelcolor="black", fontsize=9, loc="upper center",
                     ncol=3)
                     
    for sp in ["top", "right"]:
        ax2.spines[sp].set_visible(False)
    ax2.spines["bottom"].set_color("black"); ax2.spines["left"].set_visible(False)
...
def plot_continuous_learning(
    csv_path     : str,
    smooth_window: int = 10,  # Decreased from 50 to 10 to capture early patterns
    run_name     : str | None = None,
    save_path    : str | None = None,
):
    """
    Plot training curves: episode reward, success rate, licks, successes, and intra-trial duration.
    Now optimized with a clean white theme.
    """
    from pathlib import Path
    ...
    # Set outer figure background to white
    fig, axes = plt.subplots(3, 1, figsize=(8, 6), facecolor="white", sharex=True)
    axes[0].set_box_aspect(0.75)
    ...
    for ax, (key, ylabel, col) in zip(axes, metrics):
        # Inner plot background to white
        ax.set_facecolor("white")
        
        # Black axes lines and text
        for sp in ["top", "right"]: ax.spines[sp].set_visible(False)
        ...
        if key not in data:
            ax.text(0.5, 0.5, f"'{key}' not in data", ha="center", va="center",
                   color="#D00000", transform=ax.transAxes)
        ...
        if key == "intra_trial_duration":
            ...
            if len(valid_vals) > 0:
                # Raw data line (slightly more opaque for white bg)
                ax.plot(valid_ep, valid_vals, alpha=0.3, color=col, lw=1.0)
                if len(valid_vals) >= smooth_window:
                    s = _smooth(valid_vals, smooth_window)
                    ax.plot(valid_ep[smooth_window-1:], s, color=col, lw=2.5)
        else:
            # Raw data line
            ax.plot(ep, vals, alpha=0.3, color=col, lw=1.0)
            if len(vals) >= smooth_window:
                s = _smooth(vals, smooth_window)
                # Smoothed bold line
                ax.plot(ep[smooth_window-1:], s, color=col, lw=2.5)
        
        ax.set_ylabel(ylabel, color="black")

    axes[-1].set_xlabel("Episode", color="black")
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    run_name2 = r"results\sac_remapping_seed42_20260629_163250"
    csv_path2 = root_dir /  f"{run_name2}.csv"
    plot_continuous_learning(csv_path2, smooth_window=2, run_name=run_name2)



    run_name3 = r"results\sac_remapping_seed42_20260629_163438"
